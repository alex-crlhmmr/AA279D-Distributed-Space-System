...
function statedot = getStatedot(t, state, const, body)
    ...
    % Deputy state relative to chief in RTN
    rho  = state(7:9);      
    drho = state(10:12);   
    x = rho(1); y = rho(2); z = rho(3);
    dx = drho(1); dy = drho(2); dz = drho(3);

    % Chief Acceleration with J2
    z0 = r0(3); r0_norm2 = r0_norm^2;
    J2_fac_0 = 1.5 * J2 * mu * R^2 / r0_norm^5;
    ax_J2_0 = J2_fac_0 * r0(1) * (5*z0^2 / r0_norm2 - 1);
    ay_J2_0 = J2_fac_0 * r0(2) * (5*z0^2 / r0_norm2 - 1);
    az_J2_0 = J2_fac_0 * r0(3) * (5*z0^2 / r0_norm2 - 3);
    a0_J2 = [ax_J2_0; ay_J2_0; az_J2_0];

    % Chief EOM
    a0 = -mu / r0_norm^3 * r0 + a0_J2;
    statedot(1:3) = v0;
    statedot(4:6) = a0;
    ...
    % Central Gravity Relative Acceleration (nonlinear)
    denom = ((r0_norm + x)^2 + y^2 + z^2)^(3/2);

    ax =  2*theta_dot*dy + theta_ddot*y + theta_dot^2*x ...
        - mu * ((r0_norm + x)/denom - 1/r0_norm^2);
    ay = -2*theta_dot*dx - theta_ddot*x + theta_dot^2*y ...
        - mu * y / denom;
    az = -mu * z / denom;

    a_rel = [ax; ay; az];

    % Add J2 at Deputy Position
    % 1. Get RTN basis
    r_hat = r0 / r0_norm;
    n_hat = h_vec / h;
    t_hat = cross(n_hat, r_hat);
    ...
    % 2. Convert deputy position to ECI
    rho_ECI = R_RTN2ECI * rho;
    r1 = r0 + rho_ECI;
    r1_norm = norm(r1); z1 = r1(3); r1_norm2 = r1_norm^2;

    % 3. J2 at deputy
    J2_fac_1 = 1.5 * J2 * mu * R^2 / r1_norm^5;
    ax_J2_1 = J2_fac_1 * r1(1) * (5*z1^2 / r1_norm2 - 1);
    ay_J2_1 = J2_fac_1 * r1(2) * (5*z1^2 / r1_norm2 - 1);
    az_J2_1 = J2_fac_1 * r1(3) * (5*z1^2 / r1_norm2 - 3);
    a_J2_dep_ECI = [ax_J2_1; ay_J2_1; az_J2_1];

    % 4. Convert to RTN
    % J2 at deputy and chief in ECI
    a_J2_diff_ECI = a_J2_dep_ECI - a0_J2;
    
    % Rotate differential J2 into RTN
    a_J2_diff_RTN = R_RTN2ECI' * a_J2_diff_ECI;
    


    % Deputy RTN dynamics
    statedot(7:9) = drho;
    statedot(10:12) = a_rel + a_J2_diff_RTN;
end
...
%% (e) — Compare Chief Nonlinear vs Chief FODE Propagation in ECI

% Extract chief trajectories
r0_nonlinear = state_out(:,1:3);  % Chief ECI from nonlinear (b)
v0_nonlinear = state_out(:,4:6);

r0_fode = state_ECI_0_c(:,1:3);   % Chief ECI from FODE (c)
v0_fode = state_ECI_0_c(:,4:6);

% Compute position and velocity errors
pos_error_ECI = r0_nonlinear - r0_fode;
vel_error_ECI = v0_nonlinear - v0_fode;

% Convert to km and km/s
pos_error_ECI_km = pos_error_ECI / 1e3;    % [km]
vel_error_ECI_kms = vel_error_ECI / 1e3;   % [km/s]
tb_hr = t_b / 3600;                       % [hr]

labels = {'X','Y','Z'};

% --- POSITION ERRORS ---
figure('Name','Chief ECI Position Error','Color','w');
for k = 1:3
    subplot(3,1,k);
    plot(tb_hr, pos_error_ECI_km(:,k), 'LineWidth',1.2);
    ylabel([labels{k} '-Error [km]']);
    grid on;
    if k==1, title('Chief ECI Position Error (Nonlinear vs FODE)'); end
    if k==3, xlabel('Time [hr]'); end
end

% --- VELOCITY ERRORS ---
figure('Name','Chief ECI Velocity Error','Color','w');
for k = 1:3
    subplot(3,1,k);
    plot(tb_hr, vel_error_ECI_kms(:,k), 'LineWidth',1.2);
    ylabel([labels{k} '̇-Error [km/s]']);
    grid on;
    if k==1, title('Chief ECI Velocity Error (Nonlinear vs FODE)'); end
    if k==3, xlabel('Time [hr]'); end
end
