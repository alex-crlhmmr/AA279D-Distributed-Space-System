...
for caseIdx = 1:2
    ...
    % initialize state
    [r_ECId, v_ECId] = utils.OE2ECI(alpha_d_0, mu);
    [r_ECIc, v_ECIc] = utils.OE2ECI(alpha_c,   mu);
    [rRTN,  vRTN]   = utils.ECI2RTN(r_ECIc, v_ECIc, r_ECId, v_ECId);
    state           = [r_ECIc; v_ECIc; rRTN; vRTN];
    t_prev          = 0;
    ...
    for j = 1:M
        ...
        [T, X] = ode113(@(t,x) nonlinear_state_dot(t,x), tspan, state);
    ...
    % final coast
    [T, X] = ode113(@(t,x) nonlinear_state_dot(t,x), [t_prev, tau], state);
    T_all   = [T_all;   T];
    X_all   = [X_all;   X];
    ...
    for k = 1:N
        ...
        [a,e,i,Om,w,nu] = utils.ECI2OE(r_c, v_c, mu);
        chief_OE        = [a,e,i,Om,w,nu];
        [a,e,i,Om,w,nu] = utils.ECI2OE(r_d, v_d, mu);
        deputy_OE       = [a,e,i,Om,w,nu];
